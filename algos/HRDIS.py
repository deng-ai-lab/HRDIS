...
class HRDIS(DDIM):
    ...
    def sample(self, x, y, ts, **kwargs):
        y_0 = kwargs["y_0"]
        sigma_y = self.cfg.algo.sigma_y*2
        n = x.size(0)
        H = self.H
        ...
        for ti, si in zip(reversed(ts), reversed(ss)):
            ...
            if  i == 0:
                ...
                xt = alpha_t.sqrt() * x0_pred + (1 - alpha_t).sqrt() * noise
            ...
            et, x0_hat = et.detach(), x0_hat.detach()
            ...
            if i ==0:
                ...
                #loss_noise = torch.mul((et - last_noise).detach(), x0_pred).mean()
                loss_noise = torch.mul((et - noise).detach(), x0_pred).mean()
                last_noise = et.clone()
            i+=1
            ...
            if i>1:
                ...
                loss.backward()
                #torch.nn.utils.clip_grad_norm_(mu, max_norm=0.001)
                optimizer.step()
                #ema_x = 0.99*ema_x+0.01*mu.detach()
                ema_x = mu
